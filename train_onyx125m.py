...
class TrainingConfig:
    ...
    # Precision
    bf16: bool = False  # CPU doesn't support bf16
                ...
                try:
                    ...
                    if accumulated_tokens >= self.accum_token_cap:
                        ...
                        if self.global_step % self.config.log_every == 0:
                            ...
                            print(f"   Perplexity:    {np.exp(avg_loss):.2f}", flush=True)
                            print(f"   Learning Rate: {current_lr:.6e}", flush=True)
                            print(f"   Grad Norm:     {grad_norm:.4f}", flush=True)
                            print(f"   Tokens/sec:    {tokens_per_sec:.1f}", flush=True)
...
def main():
    ...
    model = Onyx125M(model_config).to(device=device, dtype=torch.float32)
