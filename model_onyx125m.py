...
@dataclass
class OnyxConfig:
    """Configuration for Onyx 125M Dense Model"""
    # Core architecture - 125M scale
    vocab_size: int = 128258      # Hermes Llama 3 tokenizer + <eod> token
    d_model: int = 576            # Hidden dimension
    n_layers: int = 12            # Number of layers
    n_heads: int = 12             # Attention heads
    n_kv_heads: int = 3           # GQA 4:1 ratio
    d_ff: int = 2304              # FFN dimension (4x d_model)
    ...
    num_global_heads: int = 2  # Number of heads with full context
...
def create_onyx_125m(
    ...
    """
    Create and initialize Onyx 125M model
    ...
    config = OnyxConfig(
        ...
        n_layers=12,
        n_heads=12,
        n_kv_heads=3,           # GQA 4:1
        d_ff=2304,
        max_seq_len=2048,       # 2k context
    ...
    print(f"✅ Created Onyx 125M Dense Model")
    print(f"   Parameters: {num_params:,} ({num_params/1e6:.2f}M)")
    print(f"   Architecture: Dense transformer")
    print(f"   Config: d_model={config.d_model}, n_layers={config.n_layers}, n_heads={config.n_heads}, n_kv_heads={config.n_kv_heads}")
