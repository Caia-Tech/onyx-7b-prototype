...
@dataclass
class OnyxConfig:
    """Configuration for Onyx 256M Dense Model (Option B - Recommended)"""
    # Core architecture - 256M scale
    vocab_size: int = 128258      # Hermes Llama 3 tokenizer + <eod> + <pad>
    d_model: int = 960            # Hidden dimension (12 heads × 80 dim)
    n_layers: int = 12            # Number of layers
    n_heads: int = 12             # Attention heads
    n_kv_heads: int = 12          # Full attention (no GQA bottleneck)
    d_ff: int = 3840              # FFN dimension (4x d_model)
    max_seq_len: int = 2048       # 2k context
    ...
    # Position encoding
    rope_theta: float = 10000.0   # Standard RoPE for 2k context
    rope_scaling: Optional[Dict[str, Any]] = None  # Not needed for 2k
    ...
    use_varlen_flash_attn: bool = True  # Varlen FA2 fast path (GPU only)
    ...
    # Advanced features (disabled for clean first model)
    num_attention_sinks: int = 0  # Disabled (unnecessary complexity)
    windowed_attention: bool = False  # Disabled (not needed at 2k)
    window_size: int = 2048  # Unused when windowed_attention=False
    num_global_heads: int = 2  # Unused when windowed_attention=False
...
class OptimizedAttention(nn.Module):
    ...
    def forward(
        ...
        if self.config.windowed_attention and not use_cache:
            ...
            window_mask = self._build_windowed_mask(batch_size, seq_len, x.device)
...
class Onyx125M(nn.Module):
    """Onyx 125M Dense Model - Production Ready"""
...
def create_onyx_125m(
    device: str = "cuda",
    dtype: Optional[torch.dtype] = None,
    compile_model: bool = True
) -> Onyx125M:
    """
    Create and initialize Onyx 256M model (Option B - Recommended)

    Args:
        device: Device to place model on
        dtype: Data type for model weights (None=auto, bfloat16 on CUDA, float32 on CPU)
        compile_model: Whether to compile with torch.compile

    Returns:
        Initialized Onyx 256M model
    """
    # Auto-select best dtype if not specified
    if dtype is None:
        if device == "cuda" and torch.cuda.is_available():
            dtype = torch.bfloat16  # Best for modern GPUs (H100, A100, 4090)
        else:
    ...
    # Create configuration (uses OnyxConfig defaults - Option B)
    config = OnyxConfig(
    ...
    print(f"✅ Created Onyx 256M Dense Model (Option B)")
    print(f"   Parameters: {num_params:,} ({num_params/1e6:.2f}M)")
    print(f"   Architecture: Dense transformer")
    print(f"   Config: d_model={config.d_model}, n_layers={config.n_layers}, n_heads={config.n_heads} (head_dim={config.d_model//config.n_heads})")
    print(f"   Attention: Full (n_kv_heads={config.n_kv_heads}, no GQA bottleneck)")
    print(f"   RoPE theta: {config.rope_theta} (standard for 2k context)")
...
if __name__ == "__main__":
    ...
    print("Testing Onyx 125M Production Model")
    print("=" * 60)
    ...
    model = create_onyx_125m(device=device, dtype=dtype, compile_model=False)  # Skip compile for testing

    # Test forward pass
    print("\nTesting forward pass...")
    input_ids = torch.randint(0, 128256, (2, 64), device=device)
    ...
    # Without cache
    start = time.time()
    generated = model.generate(input_ids[:1], max_length=128, use_cache=False)
    time_no_cache = time.time() - start

    # With cache
    start = time.time()
    generated = model.generate(input_ids[:1], max_length=128, use_cache=True)
